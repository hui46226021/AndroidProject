 ...
 * @since 1.1
 */
class UpdateHandler extends com.jereibaselibrary.db.litepal.crud.DataHandler {
	 ...
	 */
	int onUpdate(com.jereibaselibrary.db.litepal.crud.DataSupport baseObj, long id) throws SecurityException, IllegalArgumentException,
	...
	int onUpdateAll(com.jereibaselibrary.db.litepal.crud.DataSupport baseObj, String... conditions) throws SecurityException,
        ...
        if (!supportedGenericFields.isEmpty()) {
            List<com.jereibaselibrary.db.litepal.crud.DataSupport> list = (List<com.jereibaselibrary.db.litepal.crud.DataSupport>) com.jereibaselibrary.db.litepal.crud.DataSupport.select("id").where(conditions).find(baseObj.getClass());
            if (list.size() > 0) {
                ids = new long[list.size()];
                for (int i = 0; i < ids.length; i++) {
                    com.jereibaselibrary.db.litepal.crud.DataSupport dataSupport = list.get(i);
	 ...
	 */
	private void putFieldsToDefaultValue(com.jereibaselibrary.db.litepal.crud.DataSupport baseObj, ContentValues values, long... ids) {
		String fieldName = null;
		try {
			com.jereibaselibrary.db.litepal.crud.DataSupport emptyModel = getEmptyModel(baseObj);
	...
	private int doUpdateAssociations(com.jereibaselibrary.db.litepal.crud.DataSupport baseObj, long id, ContentValues values) {
		int rowsAffected = 0;
	 ...
	 */
	private void analyzeAssociations(com.jereibaselibrary.db.litepal.crud.DataSupport baseObj) {
		try {
	 ...
	 * Unused currently.
	 */
	private void updateSelfTableForeignKey(com.jereibaselibrary.db.litepal.crud.DataSupport baseObj, ContentValues values) {
	 ...
	 * Unused currently.
	 */
	private int updateAssociatedTableForeignKey(com.jereibaselibrary.db.litepal.crud.DataSupport baseObj, long id) {
     ...
     * will be abort. Clear generic collection data while updating should use {@link com.jereibaselibrary.db.litepal.crud.DataSupport#setToDefault(String)}
     * method.
     * @param baseObj
